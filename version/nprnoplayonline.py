...
class musicFunc(QWidget, Ui_Form):
    ...
    def play(self):
        ...
        #response = requests.get('https://npr-ice.streamguys1.com/live.mp3', headers=self.headers).content
        ...
        mixer.init()
        r = response.decode('gbk')
        sound = mixer.Sound(r)  # Load the wav
        sound.play()  # Play it
    ...
    def getNprInfo(self):
        ...
        audio_orurls = re.findall('title":"(.*?)","audioTitle.*?"date":"(.*?)T.*?audio\\\\\/mp3","href":"(.*?mp3)\?',response,re.S)  # 获取音频下载链接、标题、日期
        for orurl in audio_orurls:
            data = []
            # 处理不规范的字符，windows文件名不支持部分特殊字符
            title = orurl[0].replace('\/', ' ').encode('utf-8').decode('unicode_escape').replace('\\', '').replace(':','：').replace('?', '？').replace('<', '《').replace('>', '》').replace('*', ' ').replace('\"', '\'').replace('|',' ')
            ...
            #data.append('https://npr-ice.streamguys1.com/live.mp3?ck=1653489953029')
            #audio_urls.append(data)
            self.datalist.append(data)
'''
        for num, data in enumerate(audio_urls):
            data[4] = num
            self.datalist.append(data)
'''
